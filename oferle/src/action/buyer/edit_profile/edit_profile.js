...
export  var buyer_changePassword=(state)=>{
    ...
    return dispatch=>{
        ...
        }else
        {
            ...
            fetch( constant.base_url+constant.server_url.buyer_change_password,{
                method:"POST",
                headers:{
                    'Accept': 'application/json',
                    "Content-type": "application/x-www-form-urlencoded; charset=UTF-8",
                   },
                body:"Data="+JSON.stringify({user_id:localStorage.getItem("token_id"),"current_password":state.current_password,new_password:state.new_password,user_image:state.user_image})
            })
            .then(res=>res.json())
            .then(response=>{
...
export var buyer_edit_profile_data=(state)=>{
    ...
    return (dispatch,getstate)=>{
          ...
          if(error==false)
          {
            ...
            dispatch({type:"loader",response:{loader:true,snackbar:{show:false,message:""}}})
        fetch(constant.base_url+constant.server_url.buyer_edit_profile,{
            method:"POST",
            headers:{
                'Accept': 'application/json',
                "Content-type": "application/x-www-form-urlencoded; charset=UTF-8",
