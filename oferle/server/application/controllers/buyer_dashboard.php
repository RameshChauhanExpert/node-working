...
class Buyer_dashboard extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model('buyer_dashboard_model');
	}
          ...
          if($user_detail=(array) json_decode($_POST['Data']))
                {                    
                     $response_data=fetch("user_master",$user_detail);
                       ...
                       $data=$this->buyer_dashboard_model->seller_dashboard_detail($user_detail['user_id']);
    ...
    public function edit_profile()
    {
       if($user_detail=(array) json_decode($_POST['Data']))
                {                    
                   $this->buyer_dashboard_model->edit_profile($user_detail);
    ...
    public function change_password()
    {
        if($user_detail=(array) json_decode($_POST['Data']))
                {                    
                   $response=$this->buyer_dashboard_model->change_password($user_detail);
    ...
    {
         $response=$this->buyer_dashboard_model->edit_profile_image($_FILES);
         echo json_encode($response);
    }
